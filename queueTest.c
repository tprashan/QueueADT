...
void test_Given_a_Queue_and_a_reference_to_some_data_this_reference_will_be_inserted_on_the_queue(){
	Queue new_queue = createQueue();
	int data= 10,count;
	count = insert(new_queue,&data);
	assertEqual(count, 1);
	assertEqual(*(int*)((*new_queue.Front)->data),data);
};

void test_Given_a_Queue_and_a_reference_to_some_data_this_reference_will_be_inserted_on_the_queue_front_and_Rear(){
	Queue new_queue = createQueue();
	int data= 10,count;
	count = insert(new_queue,&data);
	assertEqual(count, 1);
...
void test_insert_puts_the_data_at_Rear(){
	int data= 1,data1=10,count;
	Queue new_queue = createQueue();
	count = insert(new_queue,&data);
	count = insert(new_queue,&data1);
	assertEqual(count, 2);
	assert((*new_queue.Front)->data==&data);
	assert((*new_queue.Rear)->data==&data1);
}

void test_queue_first_inserted_value_always_will_be_front(){
	int data= 1,data1=10,data2=30,count;
	Queue new_queue = createQueue();
	count = insert(new_queue,&data);
	count = insert(new_queue,&data1);
	count = insert(new_queue,&data2);
	assertEqual(count, 3);
	assert((*new_queue.Front)->data==&data);
...
void test_deleteFront_remove_the_data_from_the_front_of_queue(){
	int data= 1,data1=10,count;
	Queue new_queue = createQueue();
	insert(new_queue,&data);
	insert(new_queue,&data1);
...
void test_deleteFront_remove_the_data_from_Front_of_queue_and_gives_count_1(){
	int data= 1,data1=10;
	Queue new_queue = createQueue();
	insert(new_queue,&data);
	insert(new_queue,&data1);
...
void test_queue_first_inserted_value_always_will_be_front_and_next_element_will_be_front_now(){
	int data= 1,data1=10,data2=30,count;
	Queue new_queue = createQueue();
	count = insert(new_queue,&data);
	count = insert(new_queue,&data1);
